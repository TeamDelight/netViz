...
function updateWindow() {
  ...
  svg.attr('width', width * 0.8).attr('height', height * 0.8);
}
...
function isDirectChild(val) {
  for (var key in root.children) {
    if (root.children[key].name == val) return true;
  }
  return false;
}
...
function tick() {
  link
    .attr('x1', function (d) {
      if (
        d.source.name.value == d.root_element &&
        isDirectChild(d.target.name)
      ) {
        return width / 2;
      }
      return d.source.x;
    ...
    .attr('y1', function (d) {
      if (
        d.target.name.value == d.root_element &&
        isDirectChild(d.target.name)
      ) {
        return height / 2;
      }
  ...
  node.on('click', function (d) {
    const cardDetails = get_hover_card(d);
    clickStatus = true;
    hovercard
      .html(cardDetails)
  ...
  node.on('dblclick', clickLink);

  if (!clickStatus) {
    node.on('mouseover', function (d) {
      hovercard.transition().duration(100).style('opacity', 1);
...
function update_nodeDistance(val) {
  ...
  force.linkDistance(newLinkDistance).start();
}

function update_attraction(val) {
  force.charge(val).start();
}
...
function get_hover_card(d) {
  let node_details;
  node_details =
    "<table class = 'hover-table'><tr><th  colspan='2' class='hover-title'><h2>" +
    d.name +
    '</h2></th></tr>';
  for (let key_value in d) {
    if (
      ...
      if (key_value.toLowerCase() === 'link') {
        node_details = `
        ${node_details} <tr class='hover-row'><td><b>
        ${key_value}:</b></td><td class='hover-row'>
        <a class='link-external' href=${d[key_value]} target="_blank">${d[key_value]}</a><br><span class='ref-text'> (double click on node to open link)</span></td></tr>`;
      } else {
        node_details = `
        ${node_details} <tr class='hover-row'><td><b>
        ${key_value}:</b></td><td class='hover-row'>
...
const clickLink = () => {
  console.log('entered double click');
  const externalLink = document.querySelector('.link-external');
  if (externalLink) {
    console.log('im here');
    externalLink.click();
  }
